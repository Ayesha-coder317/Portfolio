...
SELECT dep.department_id, dep.name, dep.hospital_id
FROM departments dep
LEFT JOIN hospitals h ON dep.hospital_id = h.hospital_id
WHERE h.hospital_id IS NULL;

SELECT r.room_id, r.room_no, r.department_id
FROM rooms r
...
SELECT pr.prescription_id, pr.patient_id
FROM prescriptions pr
LEFT JOIN patients p ON pr.patient_id = p.patient_id
WHERE p.patient_id IS NULL;

SELECT pr.prescription_id, pr.doctor_id
FROM prescriptions pr
...
SELECT
    DATE_FORMAT(appointment_date, '%Y-%m') AS appt_year_month,
...
SELECT
    ...
    ) AS doctor_rank_in_hospital
FROM doctor_appt_counts
ORDER BY hospital_name, doctor_rank_in_hospital, doctor_name;
...
WITH med_counts AS (
    SELECT
        ...
        COUNT(p.prescription_id) AS freq
    FROM medications m
    LEFT JOIN prescriptions p
...
SELECT
    mc.medication_id,
    mc.medication_name,
    mc.freq AS prescription_frequency
FROM med_counts mc
JOIN top_freqs tf
    ON mc.freq = tf.freq
ORDER BY mc.freq DESC, mc.medication_name;
...
WITH cardio_room_counts AS (
    ...
    GROUP BY h.hospital_id, h.name, d.department_id
),
max_count AS (
    SELECT MAX(room_count) AS max_rooms
    FROM cardio_room_counts
)
SELECT 
    c.hospital_name,
    c.room_count AS cardiology_room_count
FROM cardio_room_counts c
JOIN max_count m
    ON c.room_count = m.max_rooms;
    
WITH appt_ordered AS (
    SELECT
        a.appointment_id,
        a.patient_id,
        p.name AS patient_name,
...
SELECT
    ...
    prev_appointment_date,
    DATEDIFF(appointment_date, prev_appointment_date) AS days_since_last_visit
FROM appt_ordered
WHERE prev_appointment_date IS NOT NULL
ORDER BY patient_name, appointment_date;

-- Top hospitals with the most cardiologists

SELECT 
    h.name AS hospital_name,
    COUNT(d.doctor_id) AS cardiologists_count
FROM hospitals h
JOIN doctors d ON h.hospital_id = d.hospital_id
WHERE d.specialty = 'Cardiology'
GROUP BY h.name
ORDER BY cardiologists_count DESC;
